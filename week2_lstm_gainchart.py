...
X_train = np.array([X_train[i:i+3] for i in range(len(X_train) - 3)])
y_train = y_train[3:]
X_test = np.array([X_test[i:i+3] for i in range(len(X_test) - 3)])
y_test = y_test[3:]
...
# 모델 학습
model.fit(X_train, y_train, epochs=400, batch_size=16, validation_split=0.2)

# 예측 및 평가
prob_y = model.predict(X_test)  # 양성 클래스의 확률
...
pred_y = (model.predict(X_test) > threshold).astype(int).flatten()
