...
df = df_59.drop(['날짜', '일조', '건물번호', '전력사용량', '비전기냉방설비운영', '태양광보유', '년', '날짜', '전력중앙값', '변화율'], axis=1)

# 학습 및 테스트 데이터셋 분할
# 22, 23일
# test_df = df[(df['월'] == 8) & ((df['일'] == 22)|(df['일'] == 23))]
# train_df = df[(df['월'] == 8) & (df['일'] < 22)]
...
catboost_clf = CatBoostClassifier(learning_rate=0.1, depth=6, eval_metric='AUC', random_seed=42, verbose=0)
